...
class CustomTaxonomy
{
	...
	public function create_taxonomy_metabox_markup(WP_Post $post): void
	{
		...
		$current_type = is_array($attached_type)
			? $attached_type[0]->term_id
			: -1;
	 ...
	 *
	 * @return \WP_Term|int|\P4\MasterTheme\WP_Error
	 */
		...
		if ('post' === $post->post_type) {
			...
			if (! \is_wp_error($terms)) {
				...
				if (empty($terms)) {
					...
					count($terms) > 1 && $terms[0] instanceof WP_Term
				) {
	...
	public function filter_posts_by_page_type(string $post_type, string $which): void
	{
		...
		// Display filter HTML.
		?>
		<select name="<?php echo \esc_attr(self::TAXONOMY); ?>" id="<?php echo \esc_attr(
			self::TAXONOMY,
                                                                   ); ?>" class="postform">
