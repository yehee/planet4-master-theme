...
if (strpos($post->post_content, '<!-- wp:planet4-blocks/take-action-boxout') !== false) {
	...
	$take_action_boxout_block = substr(
		$post->post_content,
		$take_action_boxout_block_start,
		$take_action_boxout_block_length,
	);
...
$page_terms_data = is_array($page_terms_data)
	? reset($page_terms_data)
	: null;
...
if ($post->include_articles === 'yes') {
	...
	$post->articles = '<!-- wp:planet4-blocks/articles ' . wp_json_encode(
		$block_attributes,
...
if (isset($take_action_boxout_block)) {
	...
	$post->take_action_boxout = '<!-- wp:planet4-blocks/take-action-boxout ' . wp_json_encode(
		$block_attributes,
		JSON_UNESCAPED_SLASHES,
	) . ' /-->';
}
